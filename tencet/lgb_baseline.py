...
def LGB_predict(train_x,train_y,test_x,res):
    ...
    clf = lgb.LGBMClassifier(
        boosting_type='gbdt', num_leaves=35, reg_alpha=0.0, reg_lambda=0.9,
        max_depth=-1, n_estimators=12000, objective='binary',
        subsample=0.7, colsample_bytree=0.7, subsample_freq=1,is_unbalance=True,
        learning_rate=0.07, min_child_weight=45, random_state=1314, n_jobs=-1
    )
    clf.fit(train_x, train_y, eval_set=[(train_x, train_y)], eval_metric='auc', verbose=100, early_stopping_rounds=500)
    res['score'] = clf.predict_proba(test_x)[:,1]
    res['score'] = res['score'].apply(lambda x: float('%.8f' % x))
    res.to_csv('1submission.csv', index=False)
    return clf
